...
def draw_graph(G, plan_assignment, unit_df, division_gdf, fig_name, geo_id ='GEOID10'):
    ...
    fig,ax = plt.subplots()
    unit_df.geometry.boundary.plot(color=None,edgecolor='k',linewidth = 2,ax=ax)
...
def num_splits(partition, unit_df, geo_id ='GEOID10', division_col = "COUNTYFP10"):
    ...
    splits = sum(df.groupby(division_col)["current"].nunique() > 1)
    return splits
...
def test():
    ...
    cddict =  recursive_tree_part(graph,range(k),df[pop_col].sum()/k,pop_col, .01, node_repeats=1)
    initial_partition = Partition(graph, cddict, updaters)
    ideal_population = sum(initial_partition["population"].values()) / len(initial_partition)
    division_proposal = partial(recom, pop_col=pop_col, pop_target=ideal_population, epsilon=0.05,  method=partial(division_bipartition_tree, division_col = division_col), node_repeats=2)
    ...
    for part in chain:
        if t%snapshot == 0:
            draw_graph(graph, part.assignment, unit_df, divisions, './chain_'+str(t)+'.png',geo_id =unit_id)
        t += 1
